...
interface RecordingDao {
    ...
    @Query("SELECT * FROM recordings WHERE deletedAt IS NULL ORDER BY dateCreated DESC")
    fun getAllRecordings(): Flow<List<Recording>>

    @Query("SELECT * FROM recordings WHERE deletedAt IS NULL AND isFavorite = 1 ORDER BY dateCreated DESC")
    fun getFavoriteRecordings(): Flow<List<Recording>>

    @Query("SELECT * FROM recordings WHERE deletedAt IS NULL AND folderId = :folderId ORDER BY dateCreated DESC")
    fun getRecordingsByFolder(folderId: String): Flow<List<Recording>>

    @Query("SELECT * FROM recordings WHERE deletedAt IS NULL AND (title LIKE '%' || :query || '%') ORDER BY dateCreated DESC")
    ...
    @Query("SELECT * FROM recordings WHERE deletedAt IS NOT NULL ORDER BY deletedAt DESC")
    suspend fun getDeletedRecordingsOnce(): List<Recording>

    @Query("DELETE FROM recordings WHERE deletedAt IS NOT NULL AND deletedAt < :cutoffMs")
